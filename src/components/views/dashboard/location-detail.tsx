...
const LocationStatsCard = () => {
  const [stats, setStats] = React.useState<Stats>({ routes: { total: 0 } });
    ...
    <Card className="bg-card text-muted-foreground w-full relative rounded-md overflow-hidden shadow-xl">
      <CardContent className="pt-6 pb-0">
        <div className="relative z-10">
          <div className="size-10 bg-emerald-100 dark:bg-emerald-900/50 rounded-full flex items-center justify-center mb-2">
            <MapPin className="size-5 text-emerald-500" />
          </div>
          <div className="space-y-1">
            <span className="text-4xl font-semibold block">{stats.routes.total}</span>
            <span className="text-gray-400 text-sm">All Locations</span>
          </div>
        </div>
